...
class Recipe(models.Model):
    ...
    slug = models.SlugField(
        ...
        # Благодаря default='' мы точно знаем, что там будет '' при первом
        # создании, и нужен он для метода, который при формировании slug
        # добавляет единицу к номеру id последнего рецепта в базе
        # для унификации ссылок. Если же предыдущих рецептов нет,
        # то он просто добавит единицу. В случае же, если создается два
        # рецепта с одинаковым именем, то ссылки у них все равно будут разными,
        # так как id всегда уникален. При этом наш метод не позволит сохранить
        # рецепт с default='', и потому в базе никогда не будет двух рецептов
        # с однаковым slug, в том числе с ''. Наш метод не позволит этому быть.
        # При этом если уже есть slug - то наш default не сработает.
        # Я замыкаю всю логику на это.
    ...
    def save(self, *args, **kwargs):
        ...
        # Если убрать Recipe, self - то теряется правильный slug при сохранении
